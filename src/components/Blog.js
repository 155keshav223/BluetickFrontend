...
const Blog = () => {
  return (
    <div className="blog-container">
      <h1 style={{color: "lightblue"}}>Explore and enjoy the blogs written by our passionate techies.</h1>
      <div className="blog-grid">
        {blogs.map((blog, index) => (
          <div className="blog-box" key={index}>
            <h2>{blog.title}</h2>
            <p>{blog.details}</p>
            <a href={blog.redirectLink} target="_blank" rel="noopener noreferrer">
              Read more
            </a>
          </div>
        ))}
